...
class NewsAction extends CommonAction {
  ...
  public function localnewslist(){
    ...
    $count = $LocalNews -> where(array('cid' => $this -> _get('id', 'intval'))) -> count('id');
    $page = new Page($count, 10);//每页10条
    $page->setConfig('header','条新闻');
    $show = $page -> show();
    $result = $LocalNews -> field('id,title,addtime') -> where(array('cid' => $this -> _get('id', 'intval'))) -> order('addtime DESC') -> limit($page -> firstRow . ',' . $page -> listRows) -> select();
  ...
  public function internationalnewslist(){
    ...
    import("ORG.Util.Page");// 导入分页类
    $count = $InternationalNews -> where(array('cid' => $this -> _get('id', 'intval'))) -> count('id');
    $page = new Page($count, 10);//每页10条
    $page->setConfig('header','条新闻');
    $show = $page -> show();
    $result = $InternationalNews -> field('id,title,addtime') -> where(array('cid' => $this -> _get('id', 'intval'))) -> order('addtime DESC') -> limit($page -> firstRow . ',' . $page -> listRows) -> select();
    $this -> assign('result', $result);
    $this -> assign('show', $show);
    $this -> display();
  }
